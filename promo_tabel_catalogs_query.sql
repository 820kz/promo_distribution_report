with agg_formats as (
	...
	where 1 = 1
	    and pa.is_arc = 0
	    --and paw.barcode is not null
	    AND pa.promo_id = %s
	GROUP BY pawr.region_code, paw.paw_id, paw.barcode)
select distinct 
	(
	SELECT
	    e.full_name
    ...
    paw.sprut_purchaise_price_wvat as "Закупочная цена с НДС Спрут, тг",
    paw.purchaise_price_wvat as "Закупочная цена с НДС, тг",
    paw.regular_price_wvat as "Регулярная розничная цена с НДС,тг",
    paw.regular_margin as "Маржа регулярная, %%",
    paw.purchase_promo_bonus as "Закупочная цена акционная с НДС,тг",
    paw.promo_price_wvat as "Акционная цена с НДС, тг",
    paw.promo_margin as "Маржа акционная, %%",
    paw.promo_bonus as "Акционный бонус от поставщика, %%",
    paw.discount_ab_difference as "Разница скидки и АБ",
    paw.promo_discount as "Скидка для покупателя, %%",
    paw.forecast_sales_unit as "Прогноз продаж, шт.",
    null as "Прогноз продаж с НДС, тг",
    null as "Потери",
    null as "Итого потери по акции с уч. ком-ции",
    null as "Компенсация от поставщика",
    smm.sm_value as "Механика акции",
    paw.one_check_limit as "Ограничение на 1 чек",
    dt.dmp_type_value as "Тип ДМП",
    paw.dmp_max_count as "Максимальное количество ДМП",
    paw.mc_number as "Номер М/С",
    paw.comments as "Комментарии",		
    paw.paw_id as "НЕ УДАЛЯТЬ"
from promo_tabel.promo_actions pa
    left join promo_tabel.promo_action_wars paw on pa.promo_id = paw.promo_id 
    left join promo_tabel.promo_action_war_regions pawr on paw.paw_id  = pawr.paw_id
    left join promo_tabel.promo_action_war_formats pawf on paw.paw_id = pawf.paw_id
    ...
    left join agg_formats af on af.region_code = pawr.region_code and af.barcode = paw.barcode and paw.paw_id = af.paw_id
