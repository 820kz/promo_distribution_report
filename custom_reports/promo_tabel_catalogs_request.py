...
# db_promo_tabel = {'db_ip': '172.16.10.68',
#  'db_port': '5432',
#  'db_service': 'portalDB',
#  'db_login': 'dmp_distributor',
#  'db_pass': 'Gttld4#k12ddrH'}
...
'Торговая марка' : 'X',
'Закупочная цена с НДС Спрут, тг' : 'Y',
'Закупочная цена с НДС, тг' : 'Z',
'Регулярная розничная цена с НДС,тг' : 'AA',
'Маржа регулярная, %' : 'AB',
'Закупочная цена акционная с НДС,тг' : 'AC',
'Акционная цена с НДС, тг' : 'AD',
'Маржа акционная, %' : 'AE',
'Акционный бонус от поставщика, %' : 'AF',
'Разница скидки и АБ' : 'AG',
'Скидка для покупателя, %' : 'AH',
'Прогноз продаж, шт.' : 'AI',
'Прогноз продаж с НДС, тг' : 'AJ',
'Потери' : 'AK',
'Итого потери по акции с уч. ком-ции' : 'AL',
'Компенсация от поставщика' : 'AM',
'Механика акции' : 'AN',
'Ограничение на 1 чек' : 'AO',
'Тип ДМП' : 'AP',
'Максимальное количество ДМП' : 'AQ',
'Номер М/С' : 'AR',
'Комментарии' : 'AS',
'НЕ УДАЛЯТЬ' : 'AT'}
#%%
...
def sexy_xlsx(table: pd.DataFrame, iol):
    ...
    for col in [colms['Торговая марка'], colms['Закупочная цена с НДС Спрут, тг'], colms['Закупочная цена с НДС, тг'],
                 colms['Регулярная розничная цена с НДС,тг'], colms['Маржа регулярная, %'], colms['Закупочная цена акционная с НДС,тг'], 
                 colms['Акционная цена с НДС, тг'], colms['Маржа акционная, %'], colms['Акционный бонус от поставщика, %'], 
                 colms['Разница скидки и АБ'], colms['Скидка для покупателя, %'], colms['Прогноз продаж, шт.'], 
                 colms['Прогноз продаж с НДС, тг'], colms['Потери'], colms['Итого потери по акции с уч. ком-ции'], 
                 colms['Компенсация от поставщика'], colms['Механика акции'], colms['Ограничение на 1 чек'], 
                 colms['Тип ДМП'], colms['Максимальное количество ДМП'], colms['Номер М/С'], colms['Комментарии'], 
                 colms['НЕ УДАЛЯТЬ']]:
    ...
    # Колонки с расчетами
    shops_bordering(cities = None, color = None, shop_filter = False, target_columns = [colms['Закупочная цена с НДС Спрут, тг'], colms['НЕ УДАЛЯТЬ']])
    ...
    # Модули
    for cell in range(0, last_values_row - header_row - 1):
        if df3[f'{colms["Потери"]}{first_values_row+cell}'].value == 1:
    ...
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Закупочная цена с НДС Спрут, тг"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Закупочная цена с НДС Спрут, тг"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Закупочная цена с НДС, тг
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')
        if df3[f'{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}'].value != df3[f'{colms["Закупочная цена с НДС Спрут, тг"]}{first_values_row+cell}'].value:
            df3[f'{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}'].fill = PatternFill(start_color='f2b0b0', end_color='f2b0b0', fill_type='solid')
    ...
    for cell in range(0, last_values_row - header_row + 1):
        if cell < last_values_row - header_row - 1:
            ...
            df3[f'{colms["Регулярная розничная цена с НДС,тг"]}{first_values_row+cell}'].number_format = '#,##0'
    ...
    for cell in range(0, last_values_row - header_row + 1):
        if cell < last_values_row - header_row - 1:
            df3[f'{colms["Маржа регулярная, %"]}{first_values_row+cell}'] = f'=IFERROR(({colms["Регулярная розничная цена с НДС,тг"]}{first_values_row+cell}-{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell})/{colms["Регулярная розничная цена с НДС,тг"]}{first_values_row+cell},0)'
            df3[f'{colms["Маржа регулярная, %"]}{first_values_row+cell}'].number_format = '0.0%'
            df3[f'{colms["Маржа регулярная, %"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Акционная цена с НДС, тенге
    for cell in range(0, last_values_row - header_row + 1):
        if cell < last_values_row - header_row - 1:
            #df3[f'{colms["Акционная цена с НДС, тг"]}{first_values_row+cell}'].number_format = '_-* # ##0_-;-* # ##0_-;_-* "-"??_-;_-@_-'
            df3[f'{colms["Акционная цена с НДС, тг"]}{first_values_row+cell}'].number_format = '#,##0'
    ...
    # Закупочная цена акционная с НДС, тенге
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Закупочная цена акционная с НДС,тг"]}{first_values_row+cell}'] = f'=IFERROR({colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}-({colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}*{colms["Акционный бонус от поставщика, %"]}{first_values_row+cell}/100),0)'
        df3[f'{colms["Закупочная цена акционная с НДС,тг"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Закупочная цена акционная с НДС,тг"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Маржа акционная, % 
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Маржа акционная, %"]}{first_values_row+cell}'] = f'=IFERROR(({colms["Акционная цена с НДС, тг"]}{first_values_row+cell}-{colms["Закупочная цена акционная с НДС,тг"]}{first_values_row+cell})/{colms["Акционная цена с НДС, тг"]}{first_values_row+cell},0)'
        df3[f'{colms["Маржа акционная, %"]}{first_values_row+cell}'].number_format = '0.0%'
        df3[f'{colms["Маржа акционная, %"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Разница скидки и АБ 
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Разница скидки и АБ"]}{first_values_row+cell}'] = f'=IFERROR(({colms["Скидка для покупателя, %"]}{first_values_row+cell}-{colms["Акционный бонус от поставщика, %"]}{first_values_row+cell}/100),0)'
        df3[f'{colms["Разница скидки и АБ"]}{first_values_row+cell}'].number_format = '0.0%'
        df3[f'{colms["Разница скидки и АБ"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Прогноз продаж с НДС, тенге
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Прогноз продаж с НДС, тг"]}{first_values_row+cell}'] = f'=IFERROR({colms["Акционная цена с НДС, тг"]}{first_values_row+cell}*{colms["Прогноз продаж, шт."]}{first_values_row+cell},0)'
        df3[f'{colms["Прогноз продаж с НДС, тг"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Прогноз продаж с НДС, тг"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Потери
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Потери"]}{first_values_row+cell}'] = f'=IFERROR(({colms["Акционная цена с НДС, тг"]}{first_values_row+cell}-{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell})*{colms["Прогноз продаж, шт."]}{first_values_row+cell},0)'
        df3[f'{colms["Потери"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Потери"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Итого потери по акции с уч. ком-ции
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Итого потери по акции с уч. ком-ции"]}{first_values_row+cell}'] = f'=IFERROR({colms["Потери"]}{first_values_row+cell}+{colms["Компенсация от поставщика"]}{first_values_row+cell},0)'
        df3[f'{colms["Итого потери по акции с уч. ком-ции"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Итого потери по акции с уч. ком-ции"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Компенсация от поставщика
    for cell in range(0, last_values_row - header_row - 1): 
        df3[f'{colms["Компенсация от поставщика"]}{first_values_row+cell}'] = f'=IFERROR({colms["Прогноз продаж, шт."]}{first_values_row+cell}*{colms["Закупочная цена с НДС, тг"]}{first_values_row+cell}*({colms["Акционный бонус от поставщика, %"]}{first_values_row+cell}/100),0)'
        df3[f'{colms["Компенсация от поставщика"]}{first_values_row+cell}'].number_format = '#,##0.0'
        df3[f'{colms["Компенсация от поставщика"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')

    # Акционный бонус от поставщика, %
    for cell in range(0, last_values_row - header_row + 1):
        if cell < last_values_row - header_row - 1:
            df3[f'{colms["Акционный бонус от поставщика, %"]}{first_values_row+cell}'].number_format = '#,##0.0'
            df3[f'{colms["Акционный бонус от поставщика, %"]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')
        df3[f'{colms["Акционный бонус от поставщика, %"]}{first_values_row - 2 + cell}'].fill = PatternFill(start_color='9E5ECE', end_color='9E5ECE', fill_type='solid')
    ...
    for cell in range(0, last_values_row - header_row + 1):
        if cell < last_values_row - header_row - 1:
            #df3[f'{colms["Прогноз продаж, шт."]}{first_values_row+cell}'].number_format = '_-* # ##0_-;-* # ##0_-;_-* "-"??_-;_-@_-'
            df3[f'{colms["Прогноз продаж, шт."]}{first_values_row+cell}'].number_format = '#,##0'
            df3[f'{colms["Прогноз продаж, шт."]}{first_values_row+cell}'].alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')
        df3[f'{colms["Прогноз продаж, шт."]}{first_values_row - 2 + cell}'].fill = PatternFill(start_color='A9D08E', end_color='A9D08E', fill_type='solid')
...
def get_promo_catalogs_data(promo_id, user_name, limit):
    ...
    #select_promos_query  = open("custom_reports/promo_tabel_query.sql", encoding='utf-8', mode="r").read()
    select_promos_query  = open("custom_reports/promo_tabel_catalogs_query.sql", encoding='utf-8', mode="r").read()
    ...
    def count_values(pivoted_df, value):
        ...
        """
        return pivoted_df.groupby(['Подгруппа', 'Регион проведения акции', 'Торговая марка',
    ...
    pivoted_df['Потери'] = pivoted_df.apply(check_problematic_modules, axis=1)
