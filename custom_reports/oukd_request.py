...
def sexy_xlsx(table: pd.DataFrame, ex_region):
    ...
    'Модуль' : 'R',
    'Штрих-код' : 'S',
    'Код товара в Спрут' : 'T',
    'Наименование товара' : 'U',
    'Наименование поставщика' : 'V',
    'Торговая марка' : 'W',
    'Регулярная розничная цена с НДС,тг' : 'X',
    'Акционная цена с НДС, тг' : 'Y',
    'Скидка для покупателя, %' : 'Z',
    'Ограничение на 1 чек' : 'AA',
    'Номер М/С' : 'AB'}
    ...
    return save_virtual_workbook(wb)# wb.save(iol)


def get_oukd_data(promo_id, user_name, limit):
    ...
    select_promos_query  = open("custom_reports/oukd_query.sql", encoding='utf-8', mode="r").read()
    ...
    data['dmp_id'] = where((data['dmp_id'].notna())&(data['dmp_id'].str.upper().str.strip() != 'НЕ УЧАСТВУЕТ'), data['Код товара в Спрут'], '')
    ...
    # Оставить только гиперы
    hyper_df = pivoted_df[pivoted_df['Форматы'].str.contains('HYPER')].reset_index(drop=True)
    hyper_df['Форматы'] = 'HYPER'
    hyper_df = hyper_df[[cols for cols in hyper_df.columns if 'ТЗ\n' not in cols] + [cols for cols in hyper_df.columns if 'HYPER' in cols]]

    # Оставить только SUPER
    super_df = pivoted_df[pivoted_df['Форматы'].str.contains('SUPER')].reset_index(drop=True)
    super_df['Форматы'] = 'SUPER'
    super_df = super_df[[cols for cols in super_df.columns if 'ТЗ\n' not in cols] + [cols for cols in super_df.columns if 'SUPER' in cols]]

    # Оставить только EXPRESS
    express_df = pivoted_df[pivoted_df['Форматы'].str.contains('EXPRESS')].reset_index(drop=True)
    express_df['Форматы'] = 'EXPRESS'
    express_df = express_df[[cols for cols in express_df.columns if 'ТЗ\n' not in cols] + [cols for cols in express_df.columns if 'EXPRESS' in cols]]

    # Оставить только DAILY
    daily_df = pivoted_df[pivoted_df['Форматы'].str.contains('DAILY')].reset_index(drop=True)
    daily_df['Форматы'] = 'DAILY'
    daily_df = daily_df[[cols for cols in daily_df.columns if 'ТЗ\n' not in cols] + [cols for cols in daily_df.columns if 'DAILY' in cols]]

    # to binary flow
    #output = io.BytesIO()

    zip_io = io.BytesIO()
    zip_obj = zipfile.ZipFile(zip_io, mode="w", compression=zipfile.ZIP_DEFLATED)

    # раскидать гиперы по архихву
    for cts in hyper_df['Регион проведения акции'].unique():
        reg_cols = []
        for col in pivoted_df.columns:
            if '\n' in col and col.split('\n')[1] in mark_dict and mark_dict[col.split('\n')[1]] == cts and 'HYPER' in col:
                reg_cols.append(col)

        region_data = hyper_df[hyper_df['Регион проведения акции']==cts].reset_index(drop=True)
        ...
        region_data = region_data[[col for col in region_data.columns if col not in data['shop_name'].unique()] + reg_cols]

        mask_df = pd.DataFrame({'mask': [True] * len(region_data)})
        region_data = pd.concat([region_data, mask_df], axis=1)
        ...
        for i, row in region_data.iterrows():
            ...
            for col in region_data.columns:
                if '\n' in col and col.split('\n')[1] in mark_dict and mark_dict[col.split('\n')[1]] == region:
                    if row[col] != '':
                        region_data.at[i, 'mask'] = False
                        break

        region_data = region_data.loc[region_data['mask']==False].drop(columns=['mask']).reset_index(drop=True)

        if len(region_data)>0:
            zip_obj.writestr(f'{cts}_HYPER.xlsx', sexy_xlsx(region_data, ex_cts))

    # раскидать суперы по архихву
    for cts in super_df['Регион проведения акции'].unique():
        reg_cols = []
        for col in pivoted_df.columns:
            if '\n' in col and col.split('\n')[1] in mark_dict and mark_dict[col.split('\n')[1]] == cts and 'SUPER' in col:
                reg_cols.append(col)

        region_data = super_df[super_df['Регион проведения акции']==cts].reset_index(drop=True)
        ...
        if len(region_data)>0:
            zip_obj.writestr(f'{cts}_SUPER.xlsx', sexy_xlsx(region_data, ex_cts))

    # раскидать express по архихву
    for cts in express_df['Регион проведения акции'].unique():
        reg_cols = []
        for col in pivoted_df.columns:
            if '\n' in col and col.split('\n')[1] in mark_dict and mark_dict[col.split('\n')[1]] == cts and 'EXPRESS' in col:
                reg_cols.append(col)

        region_data = express_df[express_df['Регион проведения акции']==cts].reset_index(drop=True)
        ...
        if len(region_data)>0:
            zip_obj.writestr(f'{cts}_EXPRESS.xlsx', sexy_xlsx(region_data, ex_cts))

    # раскидать daily по архихву
    for cts in daily_df['Регион проведения акции'].unique():
        reg_cols = []
        for col in pivoted_df.columns:
            if '\n' in col and col.split('\n')[1] in mark_dict and mark_dict[col.split('\n')[1]] == cts and 'DAILY' in col:
                reg_cols.append(col)

        region_data = daily_df[daily_df['Регион проведения акции']==cts].reset_index(drop=True)
        ...
        if len(region_data)>0:
            zip_obj.writestr(f'{cts}_DAILY.xlsx', sexy_xlsx(region_data, ex_cts))

    zip_obj.close()

    return io.BytesIO(zip_io.getvalue())
