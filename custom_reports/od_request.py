...
import time
import re
from numpy import where, nan, inf
import zipfile 
import logging
...
def get_od_data(promo_id, user_name, limit):

    if type(promo_id) == int:
        promo_id = [promo_id]
    else:
        promo_id = promo_id.split(',')
    ...
    for promos in promo_id:
        promos = int(promos)
        
        select_promos_query  = open("custom_reports/od_query.sql", encoding='utf-8', mode="r").read()

        db = MagnumDB.DBConnection(**db_promo_tabel)
        data = db.select(select_promos_query%(promos, promos, user_name, user_name, user_name, lim_row))
        db.close()
        ...
        pivoted_df = pivoted_df.sort_values(['promo_id', 'Отдел', 'Группа', 'Подгруппа', 'Регион проведения акции']).reset_index(drop=True)
        ...
        def count_values(pivoted_df, value):
            ...
            """
            return pivoted_df.groupby(['promo_id', 'Регион проведения акции', 'Торговая марка',
        ...
        full_df = pd.concat([full_df, pivoted_df])
    ...
    for cts in full_df['Регион проведения акции'].unique():
        reg_cols = []
        for col in full_df.columns:
            if '\n' in col and col.split('\n')[1] in mark_dict and mark_dict[col.split('\n')[1]] == cts:
                reg_cols.append(col)

        region_data = full_df[full_df['Регион проведения акции']==cts].reset_index(drop=True)
        ...
        main_cols = [c for c in full_df.columns if c[0:3] != 'ТЗ\n'] 
        region_data = region_data[main_cols + reg_cols]

        for shop in reg_cols:
            full_shop = pd.DataFrame()
            for gg in full_df['promo_id'].unique():
                if shop in [c for c in full_df.columns if c[0:3] == 'ТЗ\n'] :
                    if len(region_data) > 0:
                        shop_data = region_data[main_cols + [shop]]
                        shop_data = shop_data[(~shop_data[shop].isin(['НЕ УЧАСТВУЕТ'])) & (shop_data[shop].notna())].reset_index(drop=True)
                        if len(shop_data) > 0:
                            sorted_values = sorted(shop_data[shop].unique(), key=custom_sort)
                            shop_data = pd.concat([shop_data[shop_data[shop] == value] for value in sorted_values]).reset_index(drop=True)
                            shop = shop.split('\n')[1] + ' ' + shop.split('\n')[2]
                            if len(shop_data) > 0:
                                full_shop = pd.concat([full_shop, shop_data])
            
            if shop == 'ШФ №999':
                shop = 'ТФ №3'
            if len(full_shop) > 0:
                zip_obj.writestr(f'{cts}/{shop}.xlsx', sexy_xlsx(full_shop, ex_cts))
