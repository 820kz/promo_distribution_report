...
'Номер М/С' : 'AR',
'Комментарии' : 'AS',
'Исключить ТК Алматы' : 'AT',
'Исключить ТК Астаны' : 'AU',
'Исключить ТК Караганды' : 'AV',
'Исключить ТК Шымкент' : 'AW',
'Исключить ТК Талдыкорган' : 'AX',
'Исключить ТК Петропавловск' : 'AY',
'Исключить ТК Кызылорда' : 'AZ',
'Исключить ТК Тараз' : 'BA',
'Исключить ТК Туркестан' : 'BB',
'Исключить ТК Усть-Каменогорск' : 'BC'}
...
def sexy_xlsx(table: pd.DataFrame, iol):
    ...
    # Колонки с расчетами
    shops_bordering(cities = None, color = None, shop_filter = False, target_columns = [colms['Закупочная цена с НДС Спрут, тг'], colms['Комментарии']])
...
def get_promo_tabel_data(promo_id, user_name, limit):
